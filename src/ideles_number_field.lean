...
def inj_units_K_f : units K → I_K_f K := units.map (inj_K_f.ring_hom K).to_monoid_hom
...
`x_v ∉ R_v` or `x⁻¹_v ∉ R_v`. -/ 
lemma I_K_f.restricted_product (x : I_K_f K) : finite ({ v : maximal_spectrum (ring_of_integers K) |
  ¬ (x.val.val v) ∈ v.adic_completion_integers K } ∪ { v : maximal_spectrum (ring_of_integers K) |
...
`x⁻¹_v ∉ R_v`. -/ 
lemma I_K.restricted_product (x : I_K K) : finite ({ v : maximal_spectrum (ring_of_integers K) |
    (¬ (x.val.1.val v) ∈ v.adic_completion_integers K) } ∪ { v : maximal_spectrum
    (ring_of_integers K) | ¬ (x.inv.1.val v) ∈ v.adic_completion_integers K }) :=
...
begin
  ...
  { v : maximal_spectrum (ring_of_integers K) | ¬ (x.val.1.val v) ∈ v.adic_completion_integers K } ∪
  { v : maximal_spectrum (ring_of_integers K) | ¬ (x.inv.1.val v) ∈ v.adic_completion_integers K },
  { intros v hv,
