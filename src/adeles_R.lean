...
we define the completion of `K` with respect to its `v`-adic valuation, denoted `adic_completion`,
...
instance tr : @topological_ring (v.adic_completion K) v.ts _ := infer_instance
instance tg : @topological_add_group (v.adic_completion K) v.ts _ := infer_instance
instance us : uniform_space (v.adic_completion K) := 
@uniform_space.completion.uniform_space K v.us'
instance ug : @uniform_add_group (v.adic_completion K) v.us _ := 
@uniform_space.completion.uniform_add_group K v.us' _ v.ug'
instance cs : complete_space (v.adic_completion K) :=
@uniform_space.completion.complete_space K v.us'
...
instance : inhabited (K_hat R K) := ⟨(λ v, 0)⟩
...
instance tr_hat : topological_ring (Π (v : maximal_spectrum R), v.adic_completion K) :=
pi.topological_ring
...
noncomputable! def group_hom_prod : add_monoid_hom (R_hat R K) (K_hat R K) := 
{ to_fun    := λ x v, x v,
  map_zero' := rfl,
...
noncomputable! def hom_prod : ring_hom (R_hat R K) (K_hat R K)  := 
{ to_fun   := λ x v, x v,
  map_one' := rfl,
...
def restricted : K_hat R K → Prop := λ x, 
 ∀ᶠ (v : maximal_spectrum R) in filter.cofinite, (x v ∈ v.adic_completion_integers K)
...
begin
  ...
  have h_subset : {v : maximal_spectrum R | ¬ (x + y) v ∈  (v.adic_completion_integers K)} ⊆
    {v : maximal_spectrum R | ¬ x v ∈ (v.adic_completion_integers K)} ∪
    {v : maximal_spectrum R | ¬ y v ∈ (v.adic_completion_integers K)},
  { intros v hv,
...
begin
  ...
  have h_empty : {v : maximal_spectrum R | 
    ¬ ((0 : v.adic_completion K) ∈ v.adic_completion_integers K)} = ∅,
...
begin
  cases x with x hx,
  have h : ∀ (v : maximal_spectrum R), (-x v ∈ v.adic_completion_integers K) ↔
...
private lemma set_cond_finite {x y: finite_adele_ring' R K} 
  {V : Π (v : maximal_spectrum R), set (v.adic_completion K)} 
  (hV_int: ∀ᶠ (v : maximal_spectrum R) in filter.cofinite, V v =
    ↑(v.adic_completion_integers K)) :
  {v : maximal_spectrum R | ¬ (x.val v ∈ v.adic_completion_integers K ∧
...
private lemma is_open_Vx  {x y: finite_adele_ring' R K}
  ...
  (hV_int: ∀ᶠ (v : maximal_spectrum R) in filter.cofinite, V v = ↑(v.adic_completion_integers K))
  (hxy : ∀ (v : maximal_spectrum R), (x.val v, y.val v) ∈
    (λ p : (v.adic_completion K) × (v.adic_completion K), p.fst + p.snd) ⁻¹' V v ) 
  {Vx : Π (v : maximal_spectrum R), set (v.adic_completion K)}
  (hVx : Vx = λ v, if ( x.val v ∈ v.adic_completion_integers K ∧
    y.val v ∈ v.adic_completion_integers K ∧ V v = v.adic_completion_integers K ) then
    (v.adic_completion_integers K) else
...
begin
  ...
  { intro v, simp only [hVx], split_ifs,
    ...
    { have h_subset : {v : maximal_spectrum R | ¬ Vx v = v.adic_completion_integers K} ⊆
        {v : maximal_spectrum R | ¬ (x.val v ∈ v.adic_completion_integers K ∧
...
private lemma is_open_Vy {x y : finite_adele_ring' R K}
  ...
  (hxy : ∀ (v : maximal_spectrum R), (x.val v, y.val v) ∈
    (λ p : (v.adic_completion K) × (v.adic_completion K), p.fst + p.snd) ⁻¹' V v ) 
  {Vy : Π (v : maximal_spectrum R), set (v.adic_completion K)}
  (hVx : Vy = λ v, if ( x.val v ∈ v.adic_completion_integers K ∧
    y.val v ∈ v.adic_completion_integers K ∧ V v = v.adic_completion_integers K ) then
    (v.adic_completion_integers K) else (classical.some (classical.some_spec 
...
begin
  ...
  { intro v, simp only [hVx], split_ifs,
    ...
    { have h_subset : {v : maximal_spectrum R | ¬ Vy v = v.adic_completion_integers K} ⊆
        {v : maximal_spectrum R | ¬ (x.val v ∈ v.adic_completion_integers K ∧
          y.val v ∈ v.adic_completion_integers K ∧ V v = v.adic_completion_integers K)},
    { intros v hv h_cond_v,
      rw mem_set_of_eq at hv,
...
begin
  ...
  have hxy' : ∀ (v : maximal_spectrum R), (x.val v, y.val v) ∈
    (λ p : (v.adic_completion K) × (v.adic_completion K), p.fst + p.snd) ⁻¹' V v :=
  λ v, (hUV _).mp hxy v,
  set cond := λ v : maximal_spectrum R, x.val v ∈ v.adic_completion_integers K ∧
  ...
  set Vx : Π (v : maximal_spectrum R), set (v.adic_completion K) := λ v, if (cond v) then
    (v.adic_completion_integers K) else
    (classical.some (is_open_prod_iff.mp (hV v) _ _ (hxy' v))) with hVx,
  set Vy : Π (v : maximal_spectrum R), set (v.adic_completion K) := λ v, if (cond v) then
    (v.adic_completion_integers K) else (classical.some (classical.some_spec
...
begin
 apply continuous_generated_from,
  ...
  set cond := λ v : maximal_spectrum R, x.val v ∈ v.adic_completion_integers K ∧
    y.val v ∈ v.adic_completion_integers K ∧ V v = v.adic_completion_integers K with h_cond,
  have hS_fin : {v : maximal_spectrum R | ¬ (cond v)}.finite := set_cond_finite R K hV_int,
  set Vx : Π (v : maximal_spectrum R), set (v.adic_completion K) := λ v, 
  if (cond v) then (v.adic_completion_integers K) else
    (classical.some (is_open_prod_iff.mp (hV v) _ _ (hxy' v))) with hVx,
  set Vy : Π (v : maximal_spectrum R), set (v.adic_completion K) := λ v, if (cond v) then
    (v.adic_completion_integers K) else
...
lemma finite_adele_ring'.is_open_integer_subring_opp : is_open
  {x : (finite_adele_ring' R K)ᵐᵒᵖ | 
    ∀ (v : maximal_spectrum R),(mul_opposite.unop x).val v ∈ v.adic_completion_integers K} :=
begin
  use {x : finite_adele_ring' R K | ∀ (v : maximal_spectrum R),
    x.val v ∈ v.adic_completion_integers K},
...
lemma inj_K_image (x : K) : set.finite { v : maximal_spectrum R |
  ¬ (coe : K → (v.adic_completion K)) x ∈ (v.adic_completion_integers K)} := 
begin
  set supp := { v : maximal_spectrum R |
  ...
  have hsubset : supp ⊆ { v : maximal_spectrum R | v.as_ideal ∣ ideal.span({d})},
  { rw h_supp,
    intros v hv,
    rw mem_set_of_eq at hv ⊢,
    ...
    have h_loc : is_localization.mk' K 
      (classical.some (maximal_spectrum.valuation_def._proof_1 x)) sx =
...
λ x, ⟨(λ v : maximal_spectrum R, (coe : K → (v.adic_completion K)) x), inj_K_image R K x⟩

lemma inj_K_apply (k : K) : inj_K R K  k =
  ⟨(λ v : maximal_spectrum R, (coe : K → (v.adic_completion K)) k), inj_K_image R K k⟩ := rfl
...
noncomputable! def diag_R : submonoid (R_hat R K) :=
...
instance : inhabited (finite_adele_ring R K) := localization.inhabited _
...
begin
  ...
  use (λ v : maximal_spectrum R, 1/(x v : v.adic_completion K)),
  ext v,
...
lemma restricted_image (x : finite_adele_ring R K) : set.finite 
  { v : maximal_spectrum R | ¬ (map_to_K_hat R K x v) ∈ (v.adic_completion_integers K)} :=
begin
  ...
  have hsubset : { v : maximal_spectrum R |
    ¬ (map_to_K_hat R K x v) ∈ (v.adic_completion_integers K)} ⊆ 
    { v : maximal_spectrum R | v.as_ideal ∣ ideal.span({d})},
  { intros v hv,
    rw mem_set_of_eq at hv ⊢,
