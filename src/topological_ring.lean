...
{ sets := {(is_localization.to_localization_map M S).to_map '' U | U ∈ B.sets},
  ...
  lemma pi.ring_filter_basis {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) :
  ring_filter_basis (Π i, X i) := 
  { sets := {S : set (Π i, X i) | ∃ (U : Π i, set (X i)) (F : finset ι), (∀ i, i ∈ F → (U i) ∈ (T i).sets) ∧ S = (F : set ι).pi U },
    nonempty := 
    begin
      set S :=  λ i : ι, 
