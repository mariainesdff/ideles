...
private def pi.ring_filter_basis.sets {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) :
  set (set (Π i, X i)) := {S : set (Π i, X i) | ∃ (U : Π i, set (X i)) (F : finset ι), (∀ i, i ∈ F → (U i) ∈ (T i).sets) ∧ S = (F : set ι).pi U }


private lemma pi.ring_filter_basis.nonempty {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) :
  (pi.ring_filter_basis.sets T).nonempty := 
begin
  set S :=  λ i : ι, 
  by { choose U hU using (T i).nonempty,
    exact U,} with hS_def,
  use ((∅ : set ι).pi S),
  rw pi.ring_filter_basis.sets,
  rw set.mem_set_of_eq,
  use [S, (∅ : finset ι)],
  split,
  { intros i hi,
...
private lemma pi.ring_filter_basis.inter_sets {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ {U V : set (Π (i : ι), X i)},
...
begin
  ...
  set W := λ (i : ι), 
  begin
    by_cases hiF : i ∈ (FU ∩ FV),
    { choose W hW_filter hW_inter using filter_basis.inter_sets (T i).to_add_group_filter_basis.to_filter_basis (h_restrU i (finset.mem_of_subset
      (finset.inter_subset_left FU FV) hiF)) (h_restrV i (finset.mem_of_subset
      (finset.inter_subset_right FU FV) hiF)),
      exact W },
    { by_cases hiFU : i ∈ FU,
      { exact SU i, },
      { by_cases hiFV : i ∈ FV,
  ...
  { intros i hiF,
    ...
    { rw dif_pos hi_inter,
      cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V  ⊆ SU i ∩ SV i) _).property with hW_sets hW_inter,
      exact hW_sets }},
...
private lemma pi.ring_filter_basis.zero {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) :
  ∀ {U : set (Π (i : ι), X i)},
    U ∈ pi.ring_filter_basis.sets T → (0 : Π (i : ι), X i) ∈ U := 
 begin
  intros U hU,
...
private lemma pi.ring_filter_basis.add {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ {U : set (Π (i : ι), X i)},
    U ∈ pi.ring_filter_basis.sets T →
    (∃ (V : set (Π (i : ι), X i))
...
begin
  ...
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_add using add_group_filter_basis.add (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) + (V i) ⊆ (S i),
  { intros i hiF,
    rw hV, 
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V + V ⊆ S i) (add_group_filter_basis.add (h_restr i hiF))).property with hV_sets hV_add,
    use [hV_sets, hV_add] },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
...
private lemma pi.ring_filter_basis.neg {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ {U : set (Π (i : ι), X i)},
    U ∈ pi.ring_filter_basis.sets T →
    (∃ (V : set (Π (i : ι), X i))
...
begin
  rintros U ⟨S, F, h_restr, hSF⟩,
  set V : (Π i, set (X i)) := λ (i : ι), 
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_neg using add_group_filter_basis.neg (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) ⊆ (λ (x : X i), -x) ⁻¹' S i,
  { intros i hiF,
    rw hV,
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V ⊆ (λ (x : X i), -x) ⁻¹' S i) (add_group_filter_basis.neg (h_restr i hiF))).property with hV_sets hV_neg,
    use [hV_sets, hV_neg] },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
  have hV_neg : ∀ (i : ι), i ∈ F → (V i) ⊆ (λ (x : X i), -x) ⁻¹' (S i) := λ i hiF, (hV_property i hiF).right,
  clear hV_property,
  use [{ p : (Π i : ι, X i) | ∀ (i : ι), i ∈ F → (p i) ∈ (V i) }, V, F],
  { split,
    { intros i hiF,
...
private lemma pi.ring_filter_basis.conj {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
...
begin
  ...
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_conj using add_group_filter_basis.conj (x i) (S i) (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) ⊆ (λ (y : X i), x i + y - x i) ⁻¹' (S i),
  { intros i hiF,
    rw hV, 
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V ⊆ (λ (x_1 : X i), x i + x_1 - x i) ⁻¹' S i) _).property with hV_sets hV_conj,
    use [hV_sets, hV_conj], },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
  have hV_conj : ∀ (i : ι), (i ∈ F) → (V i) ⊆ (λ (x_1 : X i), x i + x_1 - x i) ⁻¹' (S i) := λ i hiF, (hV_property i hiF).right,
  clear hV_property,
  use [{ p : (Π i : ι, X i) | ∀ (i : ι), i ∈ F → (p i) ∈ (V i) }, V, F],
  { split,
    { intros i hiF,
...
private lemma pi.ring_filter_basis.mul {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ {U : set (Π (i : ι), X i)},
    U ∈ pi.ring_filter_basis.sets T →
    (∃ (V : set (Π (i : ι), X i))
...
begin
  ...
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_mul using ring_filter_basis.mul (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) * (V i) ⊆ (S i),
  { intros i hiF,
    ...
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V * V ⊆ S i) (ring_filter_basis.mul (h_restr i hiF))).property with hV_sets hV_mul,
    use [hV_sets, hV_mul] },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
...
private lemma pi.ring_filter_basis.mul_left {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ (x₀ : Π (i : ι), X i) (U : set (Π (i : ι), X i)),
    U ∈ pi.ring_filter_basis.sets T →
    (∃ (V : set (Π (i : ι), X i))
...
begin
  rintros x U ⟨S, F, h_restr, hSF⟩,
  set V : Π i : ι, set (X i) := λ (i : ι), 
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_mul_left using ring_filter_basis.mul_left (x i) (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) ⊆ (λ (y : X i), x i * y) ⁻¹' (S i),
  { intros i hiF,
    rw hV, 
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V ⊆ (λ (x_1 : X i), x i * x_1) ⁻¹' S i) _).property with hV_sets hV_mul_left,
    use [hV_sets, hV_mul_left], },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
  have hV_mul_left : ∀ (i : ι), (i ∈ F) → (V i) ⊆ (λ (x_1 : X i), x i * x_1) ⁻¹' (S i) := λ i hiF, (hV_property i hiF).right,
  clear hV_property,
  use [{ p : (Π i : ι, X i) | ∀ (i : ι), i ∈ F → (p i) ∈ (V i) }, V, F],
  { split,
    { intros i hiF,
...
private lemma pi.ring_filter_basis.mul_right {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) : 
  ∀ (x₀ : Π (i : ι), X i) (U : set (Π (i : ι), X i)),
    U ∈ pi.ring_filter_basis.sets T →
    (∃ (V : set (Π (i : ι), X i))
...
begin
  ...
  begin
    by_cases hiF : i ∈ F,
    { choose V hV_filter hV_mul_left using ring_filter_basis.mul_right (x i) (h_restr i hiF),
      exact V },
    exact set.univ,
  end with hV,
  have hV_property : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets ∧ (V i) ⊆ (λ (y : X i), y * x i) ⁻¹' (S i),
  { intros i hiF,
    rw hV, 
    dsimp only,
    rw [dif_pos hiF, classical.some],
    cases (classical.indefinite_description (λ (V : set (X i)), ∃ (H : V ∈ add_group_filter_basis.to_filter_basis.sets), V ⊆ (λ (x_1 : X i), x_1 * x i) ⁻¹' S i) _).property with hV_sets hV_mul_right,
    use [hV_sets, hV_mul_right], },
  have hV_sets : ∀ (i : ι), (i ∈ F) → (V i) ∈ add_group_filter_basis.to_filter_basis.sets := λ i hiF, (hV_property i hiF).left,
  have hV_mul_right : ∀ (i : ι), (i ∈ F) → (V i) ⊆ (λ (x_1 : X i), x_1 * x i) ⁻¹' (S i) := λ i hiF, (hV_property i hiF).right,
  clear hV_property,
  use [{ p : (Π i : ι, X i) | ∀ (i : ι), i ∈ F → (p i) ∈ (V i) }, V, F],
  { split,
    { intros i hiF,
...
instance pi.ring_filter_basis {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)]
  [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) :
  ...
  /- lemma pi.ring_filter_basis.mem_sets {ι : Type*} {X : ι → Type*} [∀ i, ring (X i)] [∀ i, topological_space (X i)] [∀ i, topological_ring (X i)] (T : Π i, ring_filter_basis (X i)) (U : set (Π i, X i)) (hU : U ∈ (pi.ring_filter_basis T).sets) : 
    ∃ (V : Π i : ι, set (X i)) (F : finset ι), (∀ i : ι, i ∈ F → (V i) ∈ (T i).sets) ∧ U = (F : set ι).pi V := 
  begin
    exact hU,
  end
    -/
