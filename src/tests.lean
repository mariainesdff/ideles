...
--def B_Z_p (p : primes) := { U : set ℤ_[p] | ∃ n : ℕ,  U = metric.ball (0 : ℤ_[p]) 
--(p^(-(n : ℤ)))} --TODO : add ℤ_[p]
...
begin
  ...
  have hzU : {(z : ℤ_[p])} * (metric.ball (0 : ℤ_[p]) ((p : ℝ)^-n)) = metric.ball (0 : ℤ_[p]) ((p : ℝ)^((min 1 (-n)) - (z : ℤ_[p]).valuation)) := padic_int.mul_open_ball hz (-n),
...
begin
  ...
  set T : Π p : primes, set (ℤ_[p]) := λ p, if p ∈ F then {m p}*(S p) else {m p}*(metric.ball (0 : ℤ_[p]) ((p : ℝ)^-(-1 : ℤ))) with hT,
 ...
 /- set T' : Π p : primes, set (ℤ_[p]) := λ p, by
  ...
  { intros p hpF_union,
    ...
    { dsimp only,
      ...
      rw [if_neg hpF, padic_int.mul_open_ball (hz p) _, neg_neg, min_eq_right (le_refl (1 : ℤ)), neg_sub] }},
  rw hSF, 
  ext x,
    ...
    { intro p,
      ...
      { by_cases hpF : p ∈ F,
        ...
        { have hx_p := hx p hpF_union,
          rw [if_neg hpF, neg_neg, padic_int.mul_open_ball (padic_int.cast_ne_zero.mp (hm_p p)) 1, mem_ball_0_iff, min_eq_right (refl (1 : ℤ))] at hx_p,
          rw [normed_field.norm_div, div_le_iff (norm_pos_iff.mpr (hm_p p)), padic.norm_eq_pow_val (hm_p p),  one_mul, padic.norm_le_pow_iff_norm_lt_pow_add_one _ _, padic_int.padic_norm_e_of_padic_int (x p), ← coe_coe, ← sub_eq_neg_add, padic_int.coe_valuation (m p)],
          exact hx_p }},
      { have hm_norm : ∥(m p : ℚ_[p])∥ = 1,
        { have hpF_factors: ¬ p ∈ F_factors,
          ...
          rw hF_factors at hpF_factors,
          rw [set.finite.mem_to_finset, set.mem_set_of_eq, subtype.val_eq_coe] at hpF_factors,
          rw ← padic_int.norm_int_lt_one_iff_dvd at hpF_factors,
          rw [← hzm, inj_pnat],
          dsimp only,
          ...
          { exact h_norm_one }},
        rw [normed_field.norm_div, div_le_iff (norm_pos_iff.mpr (hm_p p)),  hm_norm, padic_int.padic_norm_e_of_padic_int (x p), mul_one],
