...
lemma restricted_product (x : finite_idele_group' R K) :
  finite ({ v : maximal_spectrum R | (¬ (x.val.val v) ∈ v.adic_completion_integers K) } ∪ 
    { v : maximal_spectrum R | ¬ (x.inv.val v) ∈ v.adic_completion_integers K }) :=
finite.union x.val.property x.inv.property

/-- For any finite idèle `x`, there are finitely many maximal ideals `v` of `R` for which
`|x_v|_v ≠ 1`. -/ 
lemma finite_exponents (x : finite_idele_group' R K) : finite
  { v : maximal_spectrum R | (valued.v (x.val.val v) : with_zero (multiplicative ℤ)) ≠ 1 } :=
begin
  have h_subset : { v : maximal_spectrum R |
    (valued.v (x.val.val v) : with_zero (multiplicative ℤ)) ≠ 1 } ⊆ 
  { v : maximal_spectrum R | ¬ (x.val.val v) ∈ v.adic_completion_integers K } ∪ 
  { v : maximal_spectrum R | ¬ (x.inv.val v) ∈ v.adic_completion_integers K },
  { intros v hv,
...
begin
  have h_subset :
    {v : maximal_spectrum R | (v.as_ideal : fractional_ideal (non_zero_divisors R) K) ^ 
...
lemma finite_support' (x : finite_idele_group' R K ) : 
  (mul_support (λ (v : maximal_spectrum R), (v.as_ideal : 
    fractional_ideal (non_zero_divisors R) K) ^ -finite_idele.to_add_valuations R K x v)).finite := 
begin
  have h : {v : maximal_spectrum R | (v.as_ideal : fractional_ideal (non_zero_divisors R) K) ^ 
    -finite_idele.to_add_valuations R K x v ≠ 1} =
    {v : maximal_spectrum R | (v.as_ideal : fractional_ideal (non_zero_divisors R) K) ^ 
      finite_idele.to_add_valuations R K x v ≠ 1},
  { ext v,
...
valuation. -/
noncomputable! def map_to_fractional_ideals.def :
  (finite_idele_group' R K) → (units (fractional_ideal (non_zero_divisors R) K)) := 
λ x, ⟨map_to_fractional_ideals.val R K x, map_to_fractional_ideals.inv R K x, 
...
{ to_fun := map_to_fractional_ideals.def R K,
  map_one' := by {
    rw [map_to_fractional_ideals.def, ← units.eq_iff, units.coe_mk, units.coe_one],
...
begin
  ...
  have h_subset :
    {x : maximal_spectrum R | ¬(valued.v (a x) : with_zero (multiplicative ℤ)) ≤ 1} ⊆ 
    {x : maximal_spectrum R | ¬(valued.v (a x) : with_zero (multiplicative ℤ)) = 1},
  { intros v hv,
...
begin
  ...
  have h_subset : {x : maximal_spectrum R |
...
lemma right_inv' {a : Π v : maximal_spectrum R, v.adic_completion K}
  (ha : ∀ᶠ v : maximal_spectrum R in filter.cofinite,
    (valued.v (a v) : with_zero (multiplicative ℤ)) = 1)
...
lemma left_inv' {a : Π v : maximal_spectrum R, v.adic_completion K}
  (ha : ∀ᶠ v : maximal_spectrum R in filter.cofinite,
    (valued.v (a v) : with_zero (multiplicative ℤ)) = 1)
  (h_ne_zero : ∀ v : maximal_spectrum R, a v ≠ 0) :
...
def idele.mk (a : Π v : maximal_spectrum R, v.adic_completion K)
  (ha : ∀ᶠ v : maximal_spectrum R in filter.cofinite,
    (valued.v (a v) : with_zero (multiplicative ℤ)) = 1)
  (h_ne_zero : ∀ v : maximal_spectrum R, a v ≠ 0) :
...
def pi.unif : Π v : maximal_spectrum R, v.adic_completion K :=
λ v : maximal_spectrum R, (coe : K → (v.adic_completion K)) 
...
begin
  simp only [ne.def, valuation.zero_iff, idele.mk'],
  intro h,
...
begin
  ...
  use {p : (finite_adele_ring' R K × (finite_adele_ring' R K)ᵐᵒᵖ) | 
    ∀ v : maximal_spectrum R, (p.1.val v) ∈ v.adic_completion_integers K ∧ 
    ((mul_opposite.unop p.2).val v) ∈ v.adic_completion_integers K},
  split,
  { have : prod.topological_space.is_open
      {p : finite_adele_ring' R K × (finite_adele_ring' R K)ᵐᵒᵖ | ∀ (v : maximal_spectrum R),
